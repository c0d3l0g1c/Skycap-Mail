...
namespace Skycap.Net.Common
{
    ...
    {
        private static SemaphoreSlim _suspensionManagerMutex = new SemaphoreSlim(1);
        ...
        public static async Task SaveAsync()
        {
            try
            {
                // Save mail header dictionary
                await StorageSettings.SaveMailHeaderDictionary();

                await _suspensionManagerMutex.WaitAsync();
                ...
                LogFile.Instance.LogInformation("", "", "Saved state.");
            }
            catch (Exception ex)
            {
            ...
            {
                _suspensionManagerMutex.Release();
            }
        }
        ...
        public static async Task RestoreAsync()
        {
            ...
            try
            {
                ...
                using (IInputStream inStream = await file.OpenSequentialReadAsync())
                {
                    ...
                    if (_sessionState.Count == 0)
                        throw new Exception("Session state is corrupted.");
                }
                ...
                LogFile.Instance.LogInformation("", "", "Restored state.");
            }
            catch (FileNotFoundException)
            {
                string message = string.Format("No state to restore.", sessionStateFilename);
                LogFile.Instance.LogWarning("", "", message);
                throw new SuspensionManagerException(new Exception(message));
            }
            catch (UnauthorizedAccessException)
            {
                LogFile.Instance.LogError("", "", "Restore state failed. Trying again.");
                goto retry;
            }
            catch (Exception e)
            {
                LogFile.Instance.LogError("", "", e.ToString());
        ...
        public static Dictionary<String, Object> SessionStateForFrame(Frame frame)
        {
            ...
            if (frameState == null || frameState.Count == 0)
            {
